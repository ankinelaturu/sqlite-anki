...
//
// Output layout: DDL + cheap (non-vector) data, then a
...
// Descriptions are standard inline SQL comments — SQLite stores the CREATE text
// (comments included) in sqlite_master, and the explorer parses them for the
// table/column tooltips. Table comment goes on the CREATE line; column comments
// trail each column (one column per line).
push(`CREATE TABLE accounts ( -- Customer companies (the CRM accounts)
...
push(`CREATE TABLE contacts ( -- People who work at the accounts
  id INTEGER PRIMARY KEY,
...
push(`CREATE TABLE interactions ( -- Historical activity log with customers
  id INTEGER PRIMARY KEY,
  account_id INTEGER, -- FK -> accounts.id
...
push(`CREATE VIRTUAL TABLE opportunities USING anki( -- Sales deals — semantically searchable
  id INTEGER PRIMARY KEY,
  account_id INTEGER, -- FK -> accounts.id
...
push(`CREATE VIRTUAL TABLE support_tickets USING anki( -- Support cases — semantically searchable
  id INTEGER PRIMARY KEY,
  account_id INTEGER, -- FK -> accounts.id
...
push(`CREATE VIRTUAL TABLE knowledge_articles USING anki( -- Knowledge base — semantically searchable
  id INTEGER PRIMARY KEY,
...
push(`CREATE VIEW pipeline AS -- Accounts joined with their open opportunities
