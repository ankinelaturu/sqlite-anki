...
### 1. Persist the HNSW graph to disk
Build the graph once and store it (in a shadow structure), so open / first-`MATCH` **reads** it
instead of rebuilding — removing the open-time CPU spike. Does not cut RAM. Complementary to
incremental insertion (now shipped, see Done): splice on write, reload on open → never a full
rebuild. **Bumps the storage format.**

### 2. Session query-embedding cache (LRU)
Reuse query embeddings *across* queries in a session (today it's per-query/per-cursor only). An
**in-memory LRU scoped per-DB** (keyed by text; the model is implied once model is per-DB, see #3).
**Not** persisted to the DB file: a query embedding is one cheap forward pass to recompute, so
disk persistence adds unbounded keys + eviction for negligible gain (unlike the graph in #1, which
is expensive to rebuild).

### 3. Per-DB configuration + core / frontend separation (CLI-readiness)
...
- **Per-DB model selection.** Lift "one model per module instance (first `Embedder::load` wins)" so
  ...
  of this.*
- **Per-DB config lives in `anki_meta`.** Model **and** int8 mode (#4) ride the same rails: chosen at
...
### 4. RAM reduction (beyond the shipped streaming redesign)
The streaming redesign (`streaming-storage.md`) already cut RAM to **rowid + embeddings + HNSW**.
- **int8 quantization — opt-in per DB** (rides the `anki_meta` per-DB config, #3). Each `float32`
  ...
  *(Binary quantization — 1 bit/dim, 32× — is the aggressive cousin; needs float reranking to stay
  accurate, so it pairs with #4-Option-B, not plain int8.)*
...
### 5. "Rebuild required" migration UX
...
Build it **once, after the format-bumping features settle** (#1 persist-graph, #3 per-DB model, #4
int8), so it's written against a stable format instead of revised each bump.

### 6. Native CLI + interactive import
...
- **Interactive import tool** (probably a TUI): pick vector columns, choose model + int8, in-place vs
  companion. Built on the shared import layer from #3.
...
- **Companion-table import (faithful retrofit).** Vectorizing in-place turns a table into an `anki`
  ...
  **JOIN** back to search. Zero fidelity loss; auto-generated.
  ...
  - **Needs no vtab changes** → a **shared-layer** capability (not CLI-only), living beside the CHECK /
    constraint-reconstruction logic; rides the import-logic-home decision (#3). Costs vs in-place: a
