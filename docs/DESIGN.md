...
One index is maintained per `TEXT VECTOR` column per virtual table. It lives in RAM and is bulk-built from the in-memory embedding cache via `Hnsw::build` on the first `MATCH` after a change; subsequent writes splice single rows in incrementally (`Hnsw::add` / `Hnsw::remove`, ~O(log N)) instead of forcing a full rebuild, and (for tables with an `INTEGER PRIMARY KEY` — a VACUUM-stable rowid) the built graph is persisted to a `<name>_anki_hnsw` shadow table so open / first-`MATCH` *reads* it instead of rebuilding. Full mechanics — build, search, incremental add/remove, and the serialized blob format — are documented in `docs/hnsw.md`.
