...
- **Shadow storage is format v6** (`storage_format` in `anki_meta`; older DBs fail `xConnect` with "rebuild required"). Per-table shadows are the parallel `<name>_anki_data` (rows + embeddings) and `<name>_anki_hnsw` (HNSW graph cache). The rowid column is a user `INTEGER PRIMARY KEY` when one is declared (`rowid == id`, VACUUM-stable, autoincrement honored), else an injected `anki_id INTEGER PRIMARY KEY` (`TableState.rowid_col` / `rowid_user_idx`); embeddings are `anki_emb_<col>` blobs; user columns are stored under their **real names** with declared type + `COLLATE`. The `<name>_anki_hnsw` cache holds the serialized graph (topology only) so first-`MATCH`-after-open reads it instead of rebuilding; it's persisted in `xSync` and rebuilt-on-miss. The `anki_` prefix is **reserved** — a user column named `anki_*` is rejected at `xCreate` (import offers a rename). Declared column constraints flow into the shadow, so **UNIQUE/CHECK/NOT NULL enforce** on greenfield tables and writes honor the SQL conflict clause (`sqlite3_vtab_on_conflict`); **DEFAULT does not** (a vtab limitation), and `CREATE INDEX`/`CREATE TRIGGER`/`ALTER TABLE` on the vtab are blocked by SQLite before the module. See `docs/streaming-storage.md`, `docs/TODO.md`.
