...
- **Shadow storage is format v7** (`storage_format` in `anki_meta`; older DBs fail `xConnect` with "rebuild required"). Per-table shadows are the parallel `<name>_anki_data` (rows + embeddings) and `<name>_anki_hnsw` (HNSW graph cache). **No injected rowid column** — the shadow stores the user's columns **verbatim** (real names, declared type, `COLLATE`, and constraints incl. any `PRIMARY KEY`) and keys on SQLite's `rowid`. A user `INTEGER PRIMARY KEY` *is* that rowid (`rowid == id`, VACUUM-stable, autoincrement honored → **pinned**); otherwise the rowid is SQLite's implicit one (**not pinned**). `TableState.rowid_user_idx` = the integer-PK column, if any (`.is_some()` == pinned). Embeddings are `anki_emb_<col>` blobs. Reserved column names: the `anki_` prefix and the rowid aliases `rowid`/`_rowid_`/`oid` (rejected at `xCreate`; import offers a rename). The `<name>_anki_hnsw` cache holds the serialized graph (topology only) so first-`MATCH`-after-open reads it instead of rebuilding — **persisted in `xSync` only for pinned tables** (an unpinned rowid can be renumbered by `VACUUM`, so its graph just rebuilds in RAM on open). Declared column constraints flow into the shadow, so **UNIQUE/CHECK/NOT NULL/PRIMARY KEY enforce** on greenfield tables and writes honor the SQL conflict clause (`sqlite3_vtab_on_conflict`); **DEFAULT does not** (a vtab limitation), and `CREATE INDEX`/`CREATE TRIGGER`/`ALTER TABLE` on the vtab are blocked by SQLite before the module. See `docs/streaming-storage.md`, `docs/TODO.md`.
...
`docs/DESIGN.md` (full spec), `design-choices.md` (rationale), `dynamic-model-loading.md` (runtime model path), `match-dsl.md` (MATCH syntax), `hybrid-filtering.md` (WHERE + MATCH pushdown correctness), `query-planning.md` (vtab planning), `hnsw.md` (the in-tree HNSW index: build/search/incremental/persistence + blob format), `metrics.md` (`anki_metrics()`), `our-findings.md` (perf/size profiling), `build-variants.md` (engine/threading builds), `streaming-storage.md` (design: cut WASM RAM by streaming shadow-table data instead of full materialization), `limitations.md` (known by-design limits: vtab constraints, imports, storage format), `TODO.md` (deferred follow-up work / roadmap).
